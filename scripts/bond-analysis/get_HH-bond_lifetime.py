...
def main(fxyz, prefix, cutoff):
    ...
    all_carbon_bonds = {}
    c_index = np.where(frames[0].get_atomic_numbers()==1)[0]
    print(c_index)
    ...
    np.savetxt(str(prefix)+'-HH-bond-life-time.dat', lifetime_counter, fmt='%i')

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-fxyz', type=str, required=True, help='Location of lammpstrj file')
    parser.add_argument('--prefix', type=str, default='ASAP', help='Filename prefix')
    parser.add_argument('--rcut', type=float, default=0.45, help='Cutoff radius/2') # HH bondlength ~ 0.8A
