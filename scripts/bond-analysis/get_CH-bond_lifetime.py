...
def main(fxyz, prefix, cutoff):
    ...
    r_cut_list =  np.ones(natoms) * cutoff
    #print(r_cut_list)
    ...
    np.savetxt(str(prefix)+'-CH-bond-life-time.dat', lifetime_counter, fmt='%i')
    ...
    np.savetxt(str(prefix)+'-CH-bond-remaining-life-time.dat', remaining_lifetime, fmt='%i')

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-fxyz', type=str, required=True, help='Location of lammpstrj file')
    parser.add_argument('--prefix', type=str, default='ASAP', help='Filename prefix')
    parser.add_argument('--rcut', type=float, default=0.57, help='Cutoff radius/2') # CH bondlength ~ 1.09A
