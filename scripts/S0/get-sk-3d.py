...
def Sk(names, q, kgrid, e_A, e_B):
    ...
    if n_A > 0:
        ...
        FTrho_A[:] = np.NaN
    if e_A != e_B:
        q_B = np.asarray([ q_now for i,q_now in enumerate(q) if names[i]==e_B ])
        n_B = len(q_B)
...
def main(sprefix="Sk", straj="C64H64-P-100-T-7000", sbins=8):
    ...
    # Outputs
    ofile_AA = open(sprefix+'-CC-real.dat',"ab")
    ofile_AB = open(sprefix+'-CH-real.dat',"ab")
    ...
    while True:
        ...
        sk_AA, sk_AB, sk_BB = Sk(names, sq, kgridbase, 'C', 'H')
